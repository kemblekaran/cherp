...
$("#city").on("change", function() {
	$.getJSON('/server/jsonfiles/area.json', function(data) {
		var jsonData = data['data'];
		var stateName = $('#stateName').val();
		var cityName = $('#city').val();
		$.each(jsonData, function(key, val) {
			if (stateName == val.state && cityName == val.city) {
				$('#area').append(
						'<option value="' + val.name + '">' + val.name
								+ '</option>');
				
				$("#stateName, #city").on("change", function() {
					$('#area option[value="' + val.name + '"]').remove();
				});
			}
